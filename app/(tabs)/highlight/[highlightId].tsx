...
export default function HighlightEditor() {
  ...
  const isEditing = !!highlightId && highlightId !== 'new'
      ...
      if (!result.canceled && result.assets.length > 0) {
        ...
        for (const asset of result.assets) {
          try {
            ...
            const localUri = FileSystem.documentDirectory + `media_${Date.now()}_${Math.random().toString(36).slice(2)}.${ext}`
